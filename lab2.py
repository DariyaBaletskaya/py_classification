...
class Distance_Calculations(object):
    @staticmethod
    ...
    def get_separating_line(dx, dy, mid_line_point):
        ...
        return {
            ...
            'coord_y': Distance_Calculations.line_equation(a, b, coord_x_array),
            'a': a,
            'b': b
        }
...
class UI(object):
    ...
    def get_color(self, x, y):
        if x is not None and y is not None:
            calculations = Distance_Calculations()
            distances = [np.array([])]
    ...
    def get_centroids(self):
        centroids = np.array([])
        for c in Data.classes:
            centroid = Distance_Calculations.find_centroid(c)
    ...
    def divide_classes(self, centroids):
        ...
        for combo in combinations(centroids, 2):
            ...
            mid_point = Distance_Calculations.get_mid_section(first_centroid, second_centroid)
            # plot center point
            self.plt.plot(mid_point[0], mid_point[1], 'og')
            separating_line = Distance_Calculations.get_separating_line(dx, dy, mid_point)
            lines = np.append(lines, separating_line)
            # plot perpendicular
            self.plt.plot(separating_line['coord_x'], separating_line['coord_y'], ":k")

        for centroid in centroids:
            Perpendiculars.get_area(lines, centroid)
